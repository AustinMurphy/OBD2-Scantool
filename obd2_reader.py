...
class OBD2reader:
    ...
    # besides these 3 public methods, the above attributes can be directly queried and/or set. 
    #  it is currently required to manually set Port...

    def connect(self):
        """ Opens serial connection to reader device"""
        if (self.Port== None):
            print "Can't connect, no serial port is defined"
            raise self.ErrorNoPortDefined("Can't connect")
        elif self.State!=0:
            print "Can't connect, reader already connected"
            raise self.ErrorAlreadyConnected("Can't connect")
        else:
            #try:
                self.Port.open()
                self.State = 1
